...
CREATE TYPE approval_process_status AS ENUM ('pending', 'approved', 'rejected');
...
COMMENT ON TABLE ticket_view_permissions IS '控制 restricted 工單的查看權限';

COMMENT ON COLUMN ticket_view_permissions.user_id IS '授權的使用者 ID (與 role_id 擇一)';

COMMENT ON COLUMN ticket_view_permissions.role_id IS '授權的角色 ID (與 user_id 擇一)';
...
COMMENT ON TABLE approval_template_step_approvers IS '定義範本步驟中的多個簽核者 (用戶或角色)';
...
COMMENT ON TABLE approval_process_steps IS '一個具體簽核流程中的每一個步驟的狀態 (匯總狀態)';

COMMENT ON COLUMN approval_process_steps.status IS '此步驟的匯總狀態 (由 approvers 狀態和 approval_type 決定)';
...
COMMENT ON COLUMN approval_process_step_approvers.role_id IS '此簽核任務指派的角色 ID';

COMMENT ON COLUMN approval_process_step_approvers.status IS '此位簽核者的獨立狀態';

COMMENT ON COLUMN approval_process_step_approvers.actioned_by_id IS '實際執行簽核動作的使用者 ID';

COMMENT ON COLUMN approval_process_step_approvers.delegated_for_id IS '此簽核所代理的使用者 ID (若非代理則為 NULL)';
