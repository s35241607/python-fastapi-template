...
def demonstrate_schemas():
    ...
    # Ticket schemas
    print("\n🎫 Ticket Schemas:")
    ticket_create = TicketCreate(title="系統錯誤修復", description="應用程式崩潰問題", user_id=1)
    print(f"  TicketCreate: {ticket_create}")

    ticket_update = TicketUpdate(status=TicketStatus.IN_PROGRESS, description="正在調查中...")
    print(f"  TicketUpdate: {ticket_update}")

    # Comment schemas
    print("\n💬 Comment Schemas:")
    comment_create = CommentCreate(content="已開始處理這個問題", user_id=1)
    ...
    try:
        # 手動建立資料結構來展示
        complex_data = {"ticket": ticket_create.model_dump(), "initial_comment": comment_create.model_dump()}
