...
ticket_categories = Table(
    "ticket_categories",
    Base.metadata,
    Column("ticket_id", BigInteger, ForeignKey(f"{settings.db_schema}.tickets.id" if settings.db_schema else "tickets.id", ondelete="CASCADE"), primary_key=True),
...
class Ticket(Base, Auditable):
    ...
    ticket_template_id = Column(BigInteger, ForeignKey(f"{settings.db_schema}.ticket_templates.id" if settings.db_schema else "ticket_templates.id", ondelete="SET NULL"))
    approval_template_id = Column(BigInteger, ForeignKey(f"{settings.db_schema}.approval_templates.id" if settings.db_schema else "approval_templates.id", ondelete="SET NULL"))
    ...
    attachments = relationship(
        "Attachment",
        primaryjoin="and_(Ticket.id == Attachment.related_id, Attachment.related_type == 'ticket')",
        cascade="all, delete-orphan",
        lazy="selectin",
    )

    view_permissions = relationship("TicketViewPermission", back_populates="ticket", cascade="all, delete-orphan")
